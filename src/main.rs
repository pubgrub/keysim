...
fn main() -> Result<()> {
    ...
    let mut num_pad = Keypad::new(keypad::PadType::Num, "Numpad".to_string());
    let mut dir_pad1 = Keypad::new(keypad::PadType::Dir, "manual".to_string());
    let mut dir_pad2 = Keypad::new(keypad::PadType::Dir, "second".to_string());
    let mut dir_pad3 = Keypad::new(keypad::PadType::Dir, "third".to_string());
    let mut binding = keypad::Device::Display(display);
    num_pad.send_to = Some(&mut binding);
    let mut binding = keypad::Device::Keypad(num_pad);
    dir_pad3.send_to = Some(&mut binding);
    let mut binding = keypad::Device::Keypad(dir_pad3);
    dir_pad2.send_to = Some(&mut binding);
    dir_pad1.send_to = Some(&mut keypad::Device::Keypad(dir_pad2));
...
fn draw(frame: &mut Frame) {
    ...
    let key_button = Layout::default();

    let pads = Layout::vertical([Constraint::Percentage(25); 4]);

    let button_width = 7;
    let button_height = 3;
    let keypad_width = button_width * 3 + 2;
    let keypad_height = button_height * 2 + 2;

    for i in 0..2 {
        let pad: &Keypad = match i {
            0 => &dir_pad1,
            1 => &dir_pad2,
            2 => &dir_pad3,
            _ => (),
    ...
    render_borders(
        "manual pad".to_string(),
        &paragraph,
        Borders::ALL,
        frame,
        Rect::new(01, 1, keypad_width, keypad_height),
    );
    render_borders(
        "second pad".to_string(),
        &paragraph,
        Borders::ALL,
        frame,
        Rect::new(1 + keypad_width + 2, 1, keypad_width, keypad_height),
    );
